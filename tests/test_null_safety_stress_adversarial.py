...
class TestValuationModelsSuiteDirectStress:
    ...
    def test_model_2_ps_margin_adjusted_loss_making(self):
        ...
        res = ValuationModelsSuite.model_2_ps_margin_adjusted(
            sales_per_share=5000.0, net_margin=-0.05, sector_ps=1.2, sector_net_margin=0.08, current_price=25000.0
        )
        assert res == 0.0

        # Zero SPS
        res_zero = ValuationModelsSuite.model_2_ps_margin_adjusted(
            sales_per_share=0.0, net_margin=0.10, sector_ps=0.0, sector_net_margin=0.0, current_price=25000.0
        )
        assert res_zero >= 0.0
    ...
    def test_model_3_p_fcf_negative_cashflow(self):
        ...
        res = ValuationModelsSuite.model_3_p_fcf(
            fcf_per_share=-2000.0, sales_per_share=10000.0, sector_pfcf=0.0, hist_pfcf=0.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_4_pb_rhodes_kropf_negative_equity(self):
        res = ValuationModelsSuite.model_4_pb_rhodes_kropf(
            bvps=-5000.0, roe=-0.20, ke=0.0, sector_pb=0.0, rkv_is_overvalued=True, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_5_p_tbv_zero_tangible_book(self):
        res = ValuationModelsSuite.model_5_p_tbv(
            tbv_per_share=0.0, bvps=0.0, roic=-0.10, wacc=0.0, sector_ptbv=0.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_6_ev_ebitda_negative_ebitda_and_high_debt(self):
        res = ValuationModelsSuite.model_6_ev_ebitda(
            ebitda=-5000000000.0, total_debt=50000000000.0, cash_and_equiv=0.0, shares_out=1.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_7_p_cf_negative_cfo(self):
        res = ValuationModelsSuite.model_7_p_cf(
            cfo_per_share=-1000.0, pat_per_share=-500.0, sector_pcf=0.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_8_p_affo_negative_affo(self):
        res = ValuationModelsSuite.model_8_p_affo(
            affo=-50000000.0, net_income=-10000000.0, shares_out=0.0, sector_paffo=0.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_9_dcf_2stage_mckinsey_negative_ebit(self):
        res = ValuationModelsSuite.model_9_dcf_2stage_mckinsey(
            ebit=-10000000.0, roic=-0.05, wacc=0.0, shares_out=0.0, cash_and_equiv=0.0, total_debt=1e9, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_10_rim_edwards_bell_ohlson_loss_making(self):
        res = ValuationModelsSuite.model_10_rim_edwards_bell_ohlson(
            book_equity=-1e8, roe_base=-0.30, ke=0.0, shares_out=0.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_11_greenwald_epv_zero_revenue(self):
        res = ValuationModelsSuite.model_11_greenwald_epv(
            revenue=0.0, ebit_margin_avg=-0.10, wacc=0.0, shares_out=0.0, current_price=25000.0
        )
        assert res >= 0.0
    ...
    def test_model_12_graham_growth_negative_eps_and_bvps(self):
        ...
        res = ValuationModelsSuite.model_12_graham_growth(
            eps_ttm=-100.0, bvps=-500.0, expected_growth_pct=-10.0, benchmark_bond_yield=0.0, current_price=25000.0
        )
        assert res == 0.0

    def test_model_13_rule_of_40_hyper_decay(self):
        res = ValuationModelsSuite.model_13_rule_of_40_growth(
            sales_per_share=0.0, rev_growth_pct=-50.0, fcf_margin_pct=-80.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_14_acquirers_multiple_negative_ebit(self):
        res = ValuationModelsSuite.model_14_acquirers_multiple(
            ebit=-5e9, revenue=0.0, net_debt=1e12, shares_out=0.0, current_price=25000.0
        )
        assert res >= 0.0
    ...
    def test_model_15_buffett_owners_earnings_zero_capex_missing_ppe(self):
        res = ValuationModelsSuite.model_15_buffett_owners_earnings(
            ...
            revenue=0.0, gross_ppe=0.0, ocf=-5e8, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_16_pharma_rnpv_empty_pipeline(self):
        res = ValuationModelsSuite.model_16_pharma_rnpv(
            base_epv_per_share=0.0, pipeline_projects=[], net_cash_per_share=-1000.0, current_price=25000.0
        )
        assert res >= 0.0

    def test_model_17_bank_equity_cash_flow_zero_rwa(self):
        res = ValuationModelsSuite.model_17_bank_equity_cash_flow(
            net_income=-1e9, rwa=0.0, book_equity=0.0, roe=-0.10, ke=0.0, shares_out=0.0, current_price=25000.0
        )
        assert res >= 0.0
...
class TestValuationEngineComprehensiveNullSafety:
    ...
    def test_valuation_with_only_a_price(self, engine):
        ...
        # A price on its own is not a valuation. Every model used to fill its
        # missing drivers with a fraction of market cap, so this payload
        # produced 22 fair values that were all multiples of the 20,000 fed in.
        assert val.composite_fair_value == 0.0
        assert all(m.status == "INSUFFICIENT_DATA" for m in val.models)
    ...
    def test_valuation_missing_rwa_specifically(self, engine):
        ...
        # RWA is the Basel II denominator. Substituting 120% of market cap for
        # it made the bank's capital adequacy a function of its share price.
        assert bank_model.status == "INSUFFICIENT_DATA"
        assert bank_model.diagnostics["imputed_drivers"] == ["rwa"]
        assert bank_model.fair_value == 0.0
    ...
    def test_valuation_missing_affo_and_pipeline_specifically(self, engine):
        ...
        assert reit_m.status == "INSUFFICIENT_DATA"
        assert "landbank" in reit_m.diagnostics["imputed_drivers"]
        assert reit_m.fair_value == 0.0
    ...
    def test_valuation_missing_rab_telecom(self, engine):
        ...
        assert tel_m.status == "INSUFFICIENT_DATA"
        assert "regulated_asset_base" in tel_m.diagnostics["imputed_drivers"]
        assert tel_m.fair_value == 0.0
    ...
    def test_valuation_across_all_18_icb_sectors(self, engine):
        ...
        for sec in sector_codes:
            ...
            # Every sector must run all 22 models without crashing, and every
            # sector must decline to value a company with no statements.
            assert val.composite_fair_value == 0.0
            assert len(val.models) == 22
            for m in val.models:
    ...
    def test_valuation_with_nan_and_inf_values(self, engine):
        ...
        # NaN/Inf inputs are sanitised to absent, which now means "no data",
        # not "substitute the market cap".
        assert val.composite_fair_value == 0.0
